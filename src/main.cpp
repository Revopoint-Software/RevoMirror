...
const int CHECK_PERMISSION_MAX_TIMES = 15;
const int CHECK_PERMISSION_INTERVAL = 1000;
...
bool requestAccessibilityPermission() {
    ...
    // 检查权限并在需要时显示提示
    bool hasPermission = AXIsProcessTrustedWithOptions(options);
...
#pragma GCC diagnostic pop
  ...
  // 可以安全使用 CGRequestScreenCaptureAccess
  bool hasPermission = CGRequestScreenCaptureAccess();
  BOOST_LOG(info) << "hasPermission " << hasPermission 
                  << "request_permissions" << config::sunshine.request_permissions
                  << "accessibilityPermission()" << checkAccessibilityPermission();
  if(!hasPermission || config::sunshine.request_permissions)
  {
      return 0;
  }
...
#else
    // 需要处理不支持的情况
    #warning "CGRequestScreenCaptureAccess requires macOS 10.15 or later"
#endif
