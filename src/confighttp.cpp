...
namespace confighttp {
  ...
  void quit(resp_https_t response, req_https_t request) {
    BOOST_LOG(info) << "receive quit request";
    if (!check_content_type(response, request, "application/json")) {
      return;
    }
    if (!authenticate(response, request)) {
    ...
    output_tree["status"] = true;
    send_response(response, output_tree);
    BOOST_LOG(info) << "start quit thread";
    std::thread t([]{
      ...
      if (GetConsoleWindow() == NULL) {
        BOOST_LOG(info) << "shutdown service";
        lifetime::exit_sunshine(ERROR_SHUTDOWN_IN_PROGRESS, true);
        return;
      }
#endif
      BOOST_LOG(info) << "shutdown sunshine";
  ...
  void getSessionState(resp_https_t response, req_https_t request) {
    if (!authenticate(response, request)) {
      return;
    }
    ...
    int state = rtsp_stream::get_session_state();

    nlohmann::json output_tree;
    output_tree["state"] = state;
    send_response(response, output_tree);
  }
  ...
  void start() {
    ...
    server.resource["^/api/sessions/state$"]["GET"] = getSessionState;
