...
namespace nvhttp {
  ...
  void save_state() {
    ...
    if (file_handler::exists(config::nvhttp.file_state)) {
      try {
        file_handler::read_json(config::nvhttp.file_state, root);
    ...
    try {
      file_handler::write_json(config::nvhttp.file_state, root);
    } catch (std::exception &e) {
  ...
  void load_state() {
    ...
    pt::ptree tree;
    try {
      file_handler::read_json(config::nvhttp.file_state, tree);
    } catch (std::exception &e) {
      BOOST_LOG(error) << "Couldn't read "sv << config::nvhttp.file_state << ": "sv << e.what();
