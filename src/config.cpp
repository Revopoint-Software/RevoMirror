...
#undef _CONVERT_2_ARG_
    ...
    video_t::dd_t::mode_remapping_t mode_remapping_from_view(const std::string_view value) {
      ...
      pt::read_json(json_stream, json_tree);
  ...
  int parse(int argc, char *argv[]) {
    ...
    for (auto x = 1; x < argc; ++x) {
      ...
      if (line == "--request-permissions"sv) {
        config::sunshine.request_permissions = true;
        return 1;
      }
#ifdef _WIN32
    ...
    try {
      ...
      if (!file_handler::exists(sunshine.config_file)) {
        std::ofstream {std::filesystem::path(sunshine.config_file)};
      }
